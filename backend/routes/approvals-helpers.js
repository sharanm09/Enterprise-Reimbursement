...
async function validateItemForAction(client, itemId, approverId, requiredStatus, notInStatuses, managerCheck = false) {
  ...
  if (managerCheck) {
    query += ` LEFT JOIN users u ON r.user_id = u.id WHERE ri.id = $1`;

    if (Array.isArray(requiredStatus)) {
      const paramOffset = params.length;
      const placeholders = requiredStatus.map((_, i) => `$${paramOffset + i + 1}`).join(', ');
      query += ` AND ri.status IN (${placeholders})`;
      params.push(...requiredStatus);
    } else {
      query += ` AND ri.status = $${params.length + 1}`;
      params.push(requiredStatus);
    }

    query += ` AND (u.manager_id = $${params.length + 1} OR (u.manager_id IS NULL OR u.manager_id = u.id))`;
