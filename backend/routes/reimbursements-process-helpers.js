...
async function processAllItems(client, reimbursementId, items, itemFilesMap, userId, organizeItemFiles, processReimbursementItem, initialItemStatus = 'pending') {
  for (let i = 0; i < items.length; i++) {
    const result = await processReimbursementItem(client, reimbursementId, items[i], i, itemFilesMap, userId, initialItemStatus);
    if (result.error) {
