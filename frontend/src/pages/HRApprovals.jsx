...
const HRApprovals = ({ user }) => {
  ...
  const fetchPendingItems = async () => {
    try {
      ...
      const response = await axios.get(`${apiUrl}/approvals/hr/pending`, {
  ...
  const fetchApprovedItems = async () => {
    try {
      const apiUrl = process.env.REACT_APP_API_URL;
      const response = await axios.get(`${apiUrl}/approvals/hr/approved`, {
        withCredentials: true
      });
  ...
  return (
    <div className="space-y-4 min-h-[64px]">
      <div className="bg-white/80 backdrop-blur-sm rounded-xl border border-slate-200 p-4 shadow-sm">
        <h1 className="text-sm font-bold text-slate-800">HR Approvals</h1>
        <p className="text-xs text-slate-600 mt-1">Review and approve reimbursement items approved by Managers</p>
      </div>
        ...
        {displayItems.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-xs text-gray-600">
              {activeTab === 'pending' ? 'No items pending HR approval' : 'No approved or rejected items'}
            </p>
          </div>
        ) : (
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="mt-1">
                    ...
                    {selectedItem.item_status === 'rejected_by_hr' && (
                      <span className="px-1.5 py-0.5 bg-red-500 text-white text-xs font-semibold rounded-full">Rejected by HR</span>
                    )}
                    {selectedItem.item_status && !['approved_by_manager', 'approved_by_hr', 'rejected_by_hr'].includes(selectedItem.item_status) && (
