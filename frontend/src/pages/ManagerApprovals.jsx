...
const ManagerApprovals = ({ user }) => {
  ...
  const fetchPendingItems = async () => {
    try {
      ...
      const response = await axios.get(`${apiUrl}/approvals/manager/pending`, {
  ...
  const fetchApprovedItems = async () => {
    try {
      const apiUrl = process.env.REACT_APP_API_URL;
      const response = await axios.get(`${apiUrl}/approvals/manager/approved`, {
        withCredentials: true
      });
  ...
  const handleApprove = async (itemId) => {
    try {
      ...
      const response = await axios.post(
        `${apiUrl}/approvals/manager/approve`,
  ...
  const handleReject = async (itemId) => {
    ...
    try {
      const apiUrl = process.env.REACT_APP_API_URL;
      const response = await axios.post(
        `${apiUrl}/approvals/manager/reject`,
  ...
  return (
    <div className="space-y-4 min-h-[64px]">
      <div className="bg-white/80 backdrop-blur-sm rounded-xl border border-slate-200 p-4 shadow-sm">
        <h1 className="text-sm font-bold text-slate-800">Manager Approvals</h1>
        <p className="text-xs text-slate-600 mt-1">Review and approve reimbursement items from your direct reports</p>
      </div>
        ...
        {displayItems.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-xs text-gray-600">
              {activeTab === 'pending' ? 'No items pending your approval' : 'No approved or rejected items'}
            </p>
          </div>
        ) : (
              ...
              <tbody>
                {displayItems.map((item) => {
                  const getStatusColor = (status) => {
                    ...
                    if (status === 'approved_by_manager') return 'bg-gradient-to-r from-green-100 to-emerald-100 text-green-700 border border-green-200';
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="mt-1">
                    ...
                    {selectedItem.item_status === 'rejected_by_manager' && (
                      <span className="px-1.5 py-0.5 bg-red-500 text-white text-xs font-semibold rounded-full">Rejected by Manager</span>
                    )}
                    {selectedItem.item_status && !['pending', 'approved_by_manager', 'rejected_by_manager'].includes(selectedItem.item_status) && (
